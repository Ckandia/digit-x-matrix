...
// This file speaks Deriv's CLASSIC v3 protocol (ticks_history, active_symbols,
// msg_type responses, a `symbol` field). The documented endpoint for that
// protocol is wss://ws.derivws.com/websockets/v3, which needs a NUMERIC
// app_id registered at api.deriv.com — a different credential from the
// alphanumeric OAuth client ID used for login. Set DERIV_WS_APP_ID to your
// numeric app_id (defaults to Deriv's shared public test id, 1089).
//
// Deriv also has a newer, separate "Options Trading API" at
// api.derivws.com/trading/v1/options — a different product with a different
// field name (`underlying_symbol` instead of `symbol`). It is NOT a fallback
// for this protocol; sending classic-shaped messages there produces silent
// timeouts, not errors, which is worse than a hard failure. Don't add it back
// as a retry candidate.
const WS_APP_ID = (process.env.DERIV_WS_APP_ID || '1089').trim();
const DERIV_WS_ENDPOINTS = [process.env.DERIV_WS_URL?.trim() || `wss://ws.derivws.com/websockets/v3?app_id=${WS_APP_ID}`];
